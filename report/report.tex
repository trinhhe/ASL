...
The first optimizations applied on top of the baseline implementation were scalar replacement and writing out the inner two by two loops, starting on line 6 of algorithm \ref{algo:propagate}, explicitly. This optimization was then used as basis for all following steps.
...
Where $\prod_{k \in N(i)} m_{ki}(x_d)$ is the same for all neighbors of a node and therefore can be calculated in the outer loop, starting on line 4 in algorithm \ref{algo:propagate}.
...
\stinline{how do i get rid of the massive space that's inserted here?}
