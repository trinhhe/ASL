...
\rh{you're right, and it saves space}
...
The algorithm\sr{I would not use the word 'algorithm' in this whole subsection 'top-n-recommendation' as I find it a bit confusing (with the propagate part which is also called algorithm). I would say that we describe the type of inputs to propagate.}\rh{if I changed it to ``we'' (we set $\phi_i$), would it be better, or worse?} represents the ratings as a bipartite graph, with users and
...
For this reason, \cref{speedupGraphPaddingBipartite} displays the relative speedup over optimisation step \opt{8} measured on complete bipartite graphs of the same amount of vertices as for the small dataset.\rh{I'm not sure we need to say it was networkx. to me it feels equivalent to saying the plots were generated by matplotlib}\sr{Is it better now. If not just change it and then delete all comments.}
