...
Unrolling the inner loop, the $j$-loop in \cref{algo:propagate}, was done once on basis of optimization 2 and once on basis of optimization 3. In both implementations, the loop was unrolled by a factor of 4. This allowed for more scalar replacement. Since the four loop iterations are independent, combined\rh{don't understand the ``combined'' here} this allows for more instruction level parallelism (ILP).
...
\item why maybe better: can use aligned loads and stores for SIMD, don't have to have a leftover loop
\item why maybe worse: up to $4/8\times$ increase in size (if nodes only have 1 neighbour)
\end{itemize}


\mypar{Vectorise \opt{6}}
As seen in figure TODO, SIMD was introduced on basis of the message product precomputation.\rh{maybe unnecessarily complicated? ``[\dots], SIMD was introduced on top of \opt{3}''?} First, Intel\rh{maybe AVX2 is more specific than Intel?} intrinsics for vector instructions were introduced that very closely followed the instruction sequence of scalar computation. This way, 8 messages were processed together per loop iteration.
...
\rhinline{I guess this is WIP, I would definitely mention the ``save only one state, $p(\text{DISLIKE}) = 1 - p(\text{LIKE})$'' trick.}
