...
elements of $V$ where each component $x_v$ is $\in X$. Given $v_i \in V$, we
...
In classical BP, it is assumed that we can explicitly express $p$ in terms of
$\phi_i$ and $\psi_{ij}$.  WIP
}
	...
	\For{$\text{iter} = 0, \ldots, \mathcal{I}$}{
		...
		\ShowLn\For{$i \in V$}{
			\For{$j \in N(i)$}{
				...
				$m'_{ij} \leftarrow m'_{ij} / \|m'_{ij}\|_1$\;
			}
		}
	}
\end{algorithm}
...
In the rest of this paper, we only focus on the propagation procedure (as
described in \cref{algo:propagate}), as it is by far the largest bottleneck.
This means for example that we analyze the runtime in terms of the size of the
constructed factor graph $G$.

\mypar{Cost analysis} we have chosen the number of flops as the cost measure. analyzing \cref{algo:propagate} gives us $\deg(v)$



\mypar{Faster algorithm}
...
\begin{figure}\centering
	\includegraphics[width = \linewidth]{img/OptimizationDiagram.png}
...
Unrolling the inner loop, the $j$-loop in \cref{algo:propagate}, was done once on basis of optimization 2 and once on basis of optimization 3. In both implementations, the loop was unrolled by a factor of 4. This allowed for more scalar replacement. Since the four loop iterations are independent, computing all of them at once allows for more instruction level parallelism (ILP).
...
In TODO1, we see no improvement in relative speedup over optimisation step \opt{8}.
The latency and throughput on the platform do not differ between unaligned and aligned vectorized load.
Therefore, using aligned loads did not benefit the performance.
Apart from that, graph padding increases the memory foot print.
In case most of vertices have a very low degree, for example lower than SIMD size, the memory overhead of padding becomes significant.
Also in this case the advantage from vectorisation diminishes as most of the computation will be handled by the left-over loops, i.e. sequentially. 
\sr{mention average degree}
For this reason, TODO2 displays the relative speedup over optimisation step \opt{8} measured on complete bipartite graphs.
\sr{mention library used for generating these bipartite graphs and sizes}
TODO2 disproves the idea that padding could result in greater relative speedup on higher degree graphs.

\mypar{Other unsuccessful optimisations}
\srinline{mention all other unsuccessful optimisations}
...
\begin{figure}\centering
	...
	\caption{Roofline plot of optimisation steps \opt{8}, \opt{3} and the baseline \opt{1} using the small dataset.
			The rooflines are with respect to scalar / vectorized peak processor performance (2 resp. 8 flops/cycle) and the bandwidth between L3 cache and memory ($\beta = 6$). \label{rooflineEndToEndSmall}}
\end{figure}

\mypar{End-to-end speedup} To see the runtime gain achieved overall, \cref{speedupEndToEndSmall} displays the relative speedup over the baseline \opt{1} on the small dataset.
The overall relative speedup lies between 100 to 500$\times$ for optimisation step \opt{8}.
...
\mypar{Richard} Did the base implementation from scratch in C in a favourable way maximizing sequential array access and using a CSR inspired format for storing incoming messages. Performed unrolling optimisation of innermost $2\times2$ loops over states and scalar replacement. Tried graph padding optimization. Implemented precomputation of product optimisation. Helped Henry implementing/improving the roofline plot. Mostly prepared the slides on cost measure, experimental setup. Performed measurements and prepared plots.

\mypar{Henry} Implemented flop counting in code. Tried out unrolling outer loop over nodes further. Tried out unrolling inner loop over neighbours of a node further. Extended flop counting with e.g. byte counting. Implemented roofline plot. Helped with preparing plots for the presentation, mainly the roofline.
