...
In this paper, we focus on single core performance where we do not concern ourselves with the convergence properties of BP and only consider running the algorithm for a fixed number of iterations, namely 10 for small datasets and 2 iterations for big datasets. \HT{is this correct?}
...
The first optimizations applied on top of the baseline implementation were scalar replacement and writing out the inner two by two loops explicitly (line 6 and 8 of \cref{algo:propagate}). 
...
Where $\prod_{k \in N(i)} m_{ki}(x_d)$ is the same for all neighbors of a node and therefore can be calculated in the outer loop (line 4 of \cref{algo:propagate}).
...
The difference in the two unrolling implementations is that thanks to optimization \opt{2}, the second version unrolls a loop with much fewer computations, which means that ILP can be expected to have less impact. But it should still clearly outperform the version with more flops.


\mypar{Unroll outer loop \opt{9}}
In a different implementation\rh{would maybe mention that it's based on 3? But maybe having the tree figure is enough} we examined what happens if the outer loop is unrolled. Specially, the $i$-loop from \cref{algo:propagate} was unrolled by a factor of 4. Since with optimization \opt{3}\rh{it's 3, right?}\HT{yes, should be 3 (I changed it)} the computation of messages product was pushed to the outer loop, it was of interest to see if unrolling this loop would allow for increased ILP. Or if, with 4 inner loops run during one unrolled iteration of the outer loop, the loss of locality would result in decreased performance. \HT{shouldn't it be 4 outer loops run during one unrolled iteration of the inner loop?}
...
\begin{figure}\centering
	\includegraphics[width=\linewidth]{img/speedup[8]GraphPadding_small.pdf}
...
\begin{figure}\centering
	\includegraphics[width=\linewidth]{img/speedup[8]GraphPadding_bipartite.pdf}
...
\begin{figure}\centering
	...
	\caption{Relative speedup of further unrolling adding on top of optimization step \opt{8} on complete bipartite graphs. \label{speedupUnsuccessful}}
\end{figure}
...
\begin{figure}\centering
	\includegraphics[width=\linewidth]{img/roofline[1][3][8]_small.pdf}
...
\begin{figure}\centering
	\includegraphics[width=\linewidth]{img/speedup[1][3][8]_small.pdf}
