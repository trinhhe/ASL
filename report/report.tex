...
\mypar{Contribution}
We provide a BP implementation based on \citet{top-n-recommendation}. On top\sr{We provide an BP implemention based on ... optimised for sequential performance. The optimisations applied range from / include ...} of that we optimise the performance by some algorithmic changes, standard C optimisation techniques that include unrolling loops and scalar replacement, decreasing memory usage and utilizing SIMD instructions. Our optimised version beats the baseline implementation and also the library \emph{libDAI} BP implementation \cite{libdai}.\sr{Our BP implementation outperforms the libDAI library when applied to the specific problem of top-n recommendation. Or something similar maybe ... idk} Our overall speedup is up to 500 times, evaluated on the MovieLens datasets \cite{movieLens}.\sr{Would remove this last sentence. Speedup is probably mentioned in abstract, results and conclusion.}
...
\rhinline{got it. do you agree with how I reformulated it now?}
...
In this paper, we focus on single-core performance where we do not concern ourselves with the convergence properties of BP and only consider running the algorithm for a fixed number of iterations, namely 10 in most cases or for some unoptimised versions on big datasets one single iteration. \sr{Would leave away what is after the comma: too much detail and redundant}
...
use in our algorithm.\sr{Maybe replace 'in the form ...' with as described by \citet{top-n-recommendation}} Then we describe what role it plays in top-N
...
\srinline{When I read it I find it enough.}
\sr{For me this is fine. The paper is about optimisation.}
...
\sr{Why not remove this space here and append the next sentece?}
...
The algorithm\sr{I would not use the word 'algorithm' in this whole subsection 'top-n-recommendation' as I find it a bit confusing (with the propagate part which is also called algorithm). I would say that we describe the type of inputs to propagate.} represents the ratings as a bipartite graph, with users and
...
paper; we use the threshold that keeps edges above user's average rating). \HT{maybe explicitly mention that our tresholding is average rating of users?}\rh{why not}We use BP to model the probability that user $t$ likes a vertex. Thus,
...
iterations (10 in our case)\sr{Information in braces might be redundant.}. Then we compute the beliefs $b_i$, sort the movie
...
described in \cref{algo:propagate}), as it is by far the largest bottleneck.\sr{Maybe move this to METHOD or RESULTS and mention that 'As we see in Section ... the propagate part turns out to be the bottleneck thus for simplicity we only do the cost analysis for this part.' Or something similar ...} To
...
\begin{align*}
	...
	&\ge 14m + 16m^2 / n \\
	&= 14m + 8m \cdot \avgdeg,
\end{align*}\sr{Include normalisation line 12?}
%
...
In the next step the algorithm was optimised, specifically the message calculation. As seen in \cref{sec:background}, the messages get calculated according to:
$$m_{ij}(x_c) = \sum_{x_d \in X} \nodepot_i(x_d)\edgepot_{ij}(x_d, x_c) \cdot \prod_{k \in N(i) \setminus j} m_{ki}(x_d)$$
This means, that for every neighbour of a node computes a product of messages from all other neighbours of a node. \HT{sounds strange maybe: ``.., that for every neighbour of a node, we compute a product...}To avoid repeating the same calculations, the product was replaced with the following:
$$\prod_{k \in N(i) \setminus j} m_{ki}(x_d) = \frac{1}{m_{kj}(x_d)} \cdot \prod_{k \in N(i)} m_{ki}(x_d)$$
Where $\prod_{k \in N(i)} m_{ki}(x_d)$ is the same for all neighbours of a node and therefore can be calculated in the outer loop, right before line~\ref{line:for-j} in \cref{algo:propagate}.
...
The difference in the two unrolling implementations is that thanks to optimisation \opt{3}, the second version unrolls a loop with much fewer computations, which means that ILP can be expected to have less impact. But it should still clearly outperform the version with more flops.
...
As seen in \cref{OptDiagram}, SIMD was introduced on top of \opt{3}. First, Intel AVX2 intrinsics for vector instructions were introduced that very closely followed the instruction sequence of scalar computation. This way, 8 messages were processed together per loop iteration.\\
...
For further improvement on top of optimisation \opt{3}, on the one hand optimisation \opt{7} additionally tried to save memory and on the other hand optimisation \opt{6} added vectorisation.
...
These complete bipartite graphs were generated using the \texttt{networkx}\sr{cite how} library for 1010 up to 10100 vertices taking steps of 1010 \cite{networkx}.\rh{I'm not sure we need to say it was networkx. to me it feels equivalent to saying the plots were generated by matplotlib}
