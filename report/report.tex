...
\sr{I think this fits into the methods section.} As mentioned in \cref{subsec:topn}\sr{Is this what you wanted RH?}, we solely optimised and measured the belief propagation procedure of \cref{algo:propagate}.
...
\end{figure}\sr{Does mentioning the assumption fit here, Richard?}\rh{yes, nice}
...
Since our test input data is sparse, we wanted test whether the negative effects on speedup are decreased on graphs of large average degrees.\sr{What do you think about the paragraph now?}\rh{much better, thanks!}
...
\sr{Maybe this sentence has become superflous now.}
\rh{yes, maybe it has, commented it out.}
...
We achieved a 100$\times$\rh{1000?}\sr{no, this is not true for all of our inputs}\rh{then maybe say 100-1000$\times$?} speedup on an Intel(R) Core(TM) i5-7200U @2.50GHz machine and on average 16.13\% vectorised peak performance on dataset which does not fit entirely into cache.
