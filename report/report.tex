...
The big dataset represents graphs of 45,729 to 221,588 vertices and considers the first 16,200, 32,400 ... 162,000 (doubling the threshold every time\rh{I thought it was just $16,200k$ for $k \in \{1, \ldots, 10\}$, not doubling?}) users based on the MovieLens 25M dataset \cite{movieLensBig}.
...
Applying optimisation \opt{2} that unrolls the two innermost loops of \cref{algo:propagate} \rh{would comment this part out, too complicated sentence: iterating over the two states} to the baseline \opt{1} gives a 4$\times$ speedup.
...
\begin{figure}\centering
	...
	% RH: tried rewording this, but don't know if it's better (I think the speedup should always be with respect to / over the thing that has speedup 1× in the picture, so baseline in this case)
...
\mypar{Reducing computation} Optimisation \opt{3} was applied on top of optimisation \opt{2} and reduced floating point operations by replacing the computation of the product at \cref{line:product-calculation} in \cref{algo:propagate} with a single division in addition to moving the product into the outermost loop.
How much of runtime improvement does optimisation step \opt{3} achieve despite the fact that on the used platform less divisions can be scheduled per cycle than additions/multiplication and that they have a greater latency than additions/multiplications \cite{optimisationManual}?\rh{I'd rephrase this to emphasize the contrast between why it could be faster (improved asymptotics) and why it could be slower (now we need division). Like (schematically), ``optimisation \opt{3} reduced floating point operations [...], but it also introduced the need for division, which is bad because [...].'' and only then maybe ask the rhetorical question (although I think we don't need it)}
\cref{precomputeSpeedupSmall} shows that on the small dataset, this achieves a speedup of at least $20\times$ with respect to optimisation \opt{2}.
It seems\rh{I'd drop the ``seems'', we can be more confident here} that there were enough neighbours for each node to compute the product over.\sr{(?)*}\rh{yeah that's the reason.}
\begin{figure}\centering
	\includegraphics[width=\linewidth]{img/speedup[2][3]_small.pdf}
	\caption{Speedup of optimisation step \opt{3} (precomputing the product)\rh{the paren is redundant?} added on top of optimisation \opt{2} on the small dataset. \label{precomputeSpeedupSmall}}
\end{figure}
...
How do these optimisations impact the speedup?\rh{the rhetorical question is probably redundant here}
...
The speedup of the combined version of both optimisations (optimisation \opt{8} in \cref{cpctVectSpeedupSmall}) is greater than each of them separately.
...
\rhinline{at this point, I would definitely mention and plot the optimizations that didn't work (i.e. unrolling and padding). Or at least mention if plotting is cumbersome.}

\rhinline{maybe add another mypar? ``big inputs'' or some such}
