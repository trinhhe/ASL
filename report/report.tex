...
\srinline{
	Recommender systems are omnipresent in today's online shopping and streaming services. 
	A scalable alternative to matrix factorization is belief propagation as it supports incremental updates of the model.
    The paper presents an efficient ... appling various optimisation ideas and techniques.
	We evaluate the performance ... and achieve a speedup of at least 100$\times$. Moreover, the effects of the optimisations on speedup grow with the input size.
	...
	What do you think?
}
\end{abstract}
...
decreasing the flop count to $C(G) = 26m + 9n$. On the other hand, it
...
As a consequence, vectorised code could use aligned instead of unaligned loads. Besides that, no small sequential left-over loop would be needed when the $j$-loop from \cref{algo:propagate} is unrolled and vectorised.
To see for every unrolling and vectorisation step whether the additional nodes or the left-over loop are the bigger overhead, the graph padding could be activated using a compiler flag (by default deactivated).
...
The difference in the two unrolling implementations is that thanks to optimisation \opt{3}, the second version unrolls a loop with much fewer computations, which means that ILP can be expected to have less impact. But it should still clearly outperform the version \opt{4} with more flops.\sr{Henry, would you say we need a plot here too?}
...
As seen in \cref{OptDiagram}\rh{do we need ``as seen''?}, SIMD was introduced on top of \opt{3}. First, Intel AVX2 intrinsics for vector instructions were introduced that very closely followed the instruction sequence of scalar computation. This way, 8 messages were processed together per loop iteration.\\
In a second step, the vector instructions were optimised with shuffling to have more sequential access of the in-messages. Additionally, the number of messages handled during one iteration was reduced to 4.\\ \st{maybe I should go into more detail here}\sr{pherhaps it would be interesting}
...
\rh{these line breaks (\\) are kind of weird, are they needed?}
...
\rhinline{Maybe Experimental evaluation?}
...
This section starts by describing the experimental setup as a first part\rh{drop `as a first part', or change to `in the first part'}..
...
\subsection{Experimental setup}\label{sec:setup} \rh{Would just say `This \namecref{sec:setup} provides details...'} The following paragraphs start with providing details about experimental setup by describing the platform and compiler used, the test inputs and how measurements were performed. 
...
Furthermore, the bandwidths for L1, L2 and L3 caches are 81, 29 and 18 bytes/cycle which are taken from\rh{$\to$ according to?} \cite{optimisationManual}.
...
and then normalising / desparsifying the user and movie identifiers separately, mapping them to $\{1, \ldots, \#\text{users}\}$ and $\{1, \ldots, \#\text{movies}\}$ respectively.\rh{added this here to clarify, but not sure if clear enough}
This section's plots display the results of either a small or a big dataset composed from inputs generated this way.
The small dataset represents graphs of 981 to 10,334 vertices and 1,038 to 83,488 edges and consists of inputs % 981, 1'879, 2'442, 3'128, 4'142, 4'847, 5'167, 5'879, 6'452, 7'272, 7'766, 8'522, 9'278, 10'334
...
\rh{This is sloppy, but I think we should mention it. If you can improve on it, please do.}As mentioned in TODO, we solely optimised and measured the belief propagation procedure of \cref{algo:propagate}. The number of iterations was set to 10 with the exception of optimisations \opt{1, 2} measured on the big dataset.
...
\end{minipage}%
\end{figure*}
\rh{Fig 4: saving memory $\to$ memory reduction?}
...
but also introduced additional divisions, which is bad because the platform can schedule less divisions per cycle than additions/multiplications and divisions have a greater latency than additions/multiplications.\rh{maybe ``deduplicate'' by saying ``division has worse latency and throughput than addition/multiplication on the target machine''}
...
This is because there are enough neighbours for each node in the dataset such that computing the product over them for a node takes more cycles than using a division.\rh{suggestion: replace with:[...] because the average degree is large enough that the cost of the additional division is offset by improved time complexity ... or some such}
...
For further improvement on top of optimisation \opt{3}, on the one hand optimisation \opt{7} additionally tried to save memory\rh{reduced memory consumption/footprint (sounds more pro :D)} and on the other hand optimisation \opt{6} added vectorisation.\rh{maybe water down: I'd drop the on the one/other hand and the ``additionally''}
We see in \cref{cpctVectSpeedupSmall} that for the small dataset there is only a relative speedup of around 1.1$\times$ with saving memory while vectorisation yields a relative speedup of at most 1.6$\times$\rh{at most sounds weak, why not rather say 1.1–1.6?} with respect to optimisation \opt{3}.
The relative speedup of the combined version of both optimisations (optimisation \opt{8} in \cref{cpctVectSpeedupSmall}) is greater than each of them separately and at least 1.3$\times$\rh{again, 1.3--2$\times$ sounds somewhat more strong to me}.
...
In \cref{speedupGraphPaddingSmall}, we see no improvement\rh{no improvement? it's worse! just roast it, I won't mind :D} in relative speedup over optimisation step \opt{8}.
...
Apart from that, graph padding increases the memory footprint.
...
Also in this case, the advantages of vectorisation diminish as most of the computation will be handled by the left-over loops, i.e. sequentially.\rh{if I understand it this sentence correctly, that's not necessarily true, the padding is done so that the size will be exactly a multiple of SIMD size, so no left-over loop is needed (in the last iteration, we just maybe deal with some dummy edges which is fine, the SIMD algo doesn't mind).}
For this reason,\rh{This sounds weird, for what reason? AFAIK the reason is only described later (we want to (dis)prove that bigger avg degree might work) and that's why it's weird. But I can't think of how to reformulate} \cref{speedupGraphPaddingBipartite} displays the relative speedup over optimisation step \opt{8} measured on complete bipartite graphs of the same amount of vertices as for the small dataset.
...
It turns out for the small dataset we used, considering all users, the average degree of a vertex is on average 165.3 and after edge thresholding 89.72 on average.\rh{how did you arrive at those numbers? My calculations say something different}
...
Optimisation \opt{13} tried to achieve the same thing by doubling the unrolling factor of the inner loop but on top of optimisation step \opt{8}.
...
\mypar{Performance \& Big inputs} Since\rh{would drop both ``since'' and make simple sentences out of this, or at least the first since.} the optimisations displayed in \cref{cpctVectSpeedupSmall} constitute the final successful steps taken and since we only discussed what happens on the small dataset so far, it is interesting to see the actual performance achieved and also how it changes for a working set that doesn't fit into cache anymore.
...
It is obvious from \cref{rooflineEndToEndSmall} that all versions reside in the compute-bound region.
...
The big dataset appears after the first peak\sr{Maybe explain that peak?}\rh{Yeah. Do you want to or should I? AFAIK the peak is there because the different dataset has different average degree when starting from small inputs, so the baseline is suddenly faster before dropping again.} in \cref{speedupEndToEnd}, after which the relative speedup scales with input size.\sr{*}
...
Recommendation of products which a specific user might be interested in plays an important role on online shopping, video and streaming platforms nowadays\rh{drop}. 
We achieved a 100$\times$\rh{1000?} speedup on an Intel(R) Core(TM) i5-7200U @2.50GHz machine and up to 16.13\% vectorised peak performance on a dataset which does not fit entirely into cache.
The speedup scales with input size such that for the input with 221,588\rh{here I'd probably round to $\approx$200,000?} vertices it exceeds 1000$\times$.\sr{should we mention this, since we scaled the measurements for the baseline?}\rh{I think the scaling up is relatively legit. After rerunning the experiments, 1 and 10 iterations behaved the same. So this is a ``real'' achievement IMO.}
...
Also, for bigger inputs advantages in efficient computation get gradually drowned out by slow memory access.
Out of all optimisation steps, replacing a product calculation with a division operation and moving the product to the outermost loop as an optimisation step achieved the greatest speedup.\sr{*}\rh{Maybe focus less on details and more on the takeaway message? hmm, but what is it? probably sth along the lines of ``the best optimisation was to improve the algorithm/time complexity'', but idk how to phrase it nicely}
There might be ways to further improve the performance our implementation since it is compute-bound and did not hit peak performance.\sr{*}\rh{would omit in this form because it's kind of negative :) but if we can formulate in some insightful manner, idk, ``we suspect we could improve it if we did this / if we had a machine with better this''}
