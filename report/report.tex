...
\mypar{Contribution}
We provide a simple BP implementation based on \cite{top-n-recommendation}. On top of that we optimize the performance by some algorithmic changes, unrolling + scalar replacement, decreasing memory storage usage and utilizing SIMD instructions on a single core. Our optimized version beats the baseline implementation and also the library \emph{libDAI} implementation \cite{libdai}. Our overall speedup is up to 500 times evaluated on the Movielens datasets \cite{movieLens}.
...
%The effect of successful optimizations described in the previous section on performance and runtime are being evaluated in this section.
This section demonstrates the incremental performance and runtime improvements achieved by successively applying succeeding optimizations.  
We start with providing details about experimental setup first, then show the results of applying optimizations and finally show a roofline plot.
...
\begin{tabularx}{\linewidth}{ 
	...
	\textbf{L1 cache} 	& 	2 x 32 KB 8-way set associative instruction cache \cite{cpuWorldSpec}	\\
						&	2 x 32 KB 8-way set associative data cache \cite{cpuWorldSpec}	 		\\
	\textbf{L2 cache}	&	2 x 256 KB 4-way set associative cache \cite{cpuWorldSpec}				\\
	\textbf{L3 cache}	&	3 MB 12-way set associative shared cache \cite{cpuWorldSpec}			\\
...
and then normalizing / desparsifing the user and movie identifers seperately.
...
\srinline{Maybe mention size in bytes of data and how it fits in cache?}
