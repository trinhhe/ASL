...
This section presents all optimization steps seen in \cref{OptDiagram}.
...
The first optimizations applied on top of the baseline implementation were scalar replacement and writing out the inner two by two loops, starting on line~\ref{line:for-xc} of \cref{algo:propagate}, explicitly. This optimization was then used as basis for all following steps.
...
Where $\prod_{k \in N(i)} m_{ki}(x_d)$ is the same for all neighbors of a node and therefore can be calculated in the outer loop, right before line~\ref{line:for-j} in \cref{algo:propagate}.
