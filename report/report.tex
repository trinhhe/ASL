...
iterations. Finally, we compute the beliefs $b_i$, sort the movie
...
\begin{minipage}[t]{\oldlinewidth}\centering
	...
	\caption{Relative speedup of further unrolling added on top of optimisation step \opt{3} by optimisations \opt{9, 10}, and optimisation \opt{13} on top of optimisation step \opt{8}, all on the small dataset. \label{speedupUnsuccessful}}
\end{minipage}%
\end{figure*}
...
We achieved a 100--1000$\times$\rh{changed from 100$\times$, revert if you don't like it} speedup on an Intel\circledR{} Core™ i5-7200U @2.50GHz machine and on average 16.13\% vectorised peak performance on dataset which does not fit entirely into cache.
