...
int main(int argc, char *argv[]) {
    ...
    myInt64 start_build, stop_build, start_prop, stop_prop;
    ...
    for (int j = 0; j < REP; j++) {
        ...
        stop_build = stop_tsc(start_build);
        total_build += (double)stop_build;
        ...
        stop_prop = stop_tsc(start_prop);
        total_prop += (double) stop_prop;
        if (j == REP-1) {
            ...
            //printf("n: %li, new size: %li\n", n, var_states.size());
            //for( size_t k = 0; k < n; ++k) //DELETE
            //    var_states.push_back(factorGraph.var(k).states());
            //printf("new size: %li\n", n, var_states.size());
        }
    }
    total_build /= REP;
    ...
    // count flops
    unsigned long long int flops_build = 0;
    unsigned long long int flops_prop = 0;
    unsigned int iterations = bpclone.Iterations();


    flops_build += 10 * ri.numRatingsTargetUser + 3; //node potentials
    flops_build += ri.numRatings + ri.numUsers; //edge threshold
    flops_build += 2; //propagation matrix


    //bp2.run flops
    size_t nrEdges = fgraphclone.nrEdges();
    size_t nrVars = fgraphclone.nrVars(); // TODO: is nrVars = n
    ...
    bforeach( const Edge &e, _updateSeq ) {
        ...
        if (fgraphclone.factor(I).vars().size() != 1) {
            ...
            bforeach( const Neighbor &j, fgraphclone.nbF(I) )
                ...
                flops_prop += prod.size();      
                }    
            //marginalize in calcNewMessage (bp.cpp:228)
            flops_prop += prod.size(); 
            flops_prop += fgraphclone.var(i).states();//normalize (bp.cpp:247)
        }
    }
    ...
    for( size_t i = 0; i < nrVars; ++i) {
        bforeach( const Neighbor &I, fgraphclone.nbV(i) )
            flops_prop++;
        flops_prop += 3*fgraphclone.var(i).states(); //normalize + dist that contains dot product between probability vectors
    }
    ...
    for( size_t k = 0; k < nrFactors; ++k) {        
        ...
        flops_prop += prod.size();
        flops_prop += 2*prod.size(); //dist (bp.cpp:332)
    }
    flops_prop *= iterations;
    double total = total_build + total_prop;
    unsigned long long total_flops = flops_build + flops_prop;
    double total_belief = 0.0;
    unsigned long long flops_belief = 0;

    printf("%zu, %f, %llu, %f, %f, %f, %llu, %llu, %llu, %d\n", nrVars, total, total_flops, total_build, total_prop, total_belief, flops_build, flops_prop, flops_belief, iterations);
